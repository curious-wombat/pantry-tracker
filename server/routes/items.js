...
router.get('/', (req, res) => {
  ...
  const params = [];
  if (location) { query += ' WHERE storage_location = ?'; params.push(location); }
...
router.delete('/:id', (req, res) => {
  if (!db.prepare('SELECT id FROM items WHERE id = ?').get(req.params.id)) return res.status(404).json({ error: 'Item not found' });
