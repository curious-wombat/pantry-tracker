...
export default function InventoryView({ items, location, setLocation, onUse, onEdit, onDelete, onAdd, lowStockCount }) {
  ...
  const lowCount = locationItems.filter(i => i.commonly_used && i.quantity <= i.low_stock_threshold).length
        ...
        <div className="flex gap-2 mb-4">
          {Object.entries(LOCATION_CONFIG).map(([loc, cfg]) => {
            ...
            return (
              ...
              >
                {cfg.emoji} {cfg.label}
                <span className={`ml-1.5 text-xs ${location === loc ? 'opacity-70' : 'opacity-50'}`}>
                  {count}
                </span>
              </button>
            )
      ...
      {lowCount > 0 && (
        <div className="mx-4 mt-3 px-4 py-2.5 bg-terra/10 border border-terra/20 rounded-xl flex items-center gap-2">
          ...
          <p className="text-terra text-sm font-medium">
            {lowCount} item{lowCount > 1 ? 's' : ''} running low in your {config.label.toLowerCase()}
          </p>
        </div>
      )}
